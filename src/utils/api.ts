...
class Api {
        ...
        for (let attempt = 0; attempt < retries; attempt++) {
            try {
                ...
                if (response.status === 200) {
                    ...
                    await Logger.warning(`🛑 Warning: Timeout detected (attempt ${attempt + 1}/${retries}). Retrying...`);
                    continue;
                } else {
                    const errorMessage = await getErrorMessage(response);
                    await Logger.error(`🛑 API Error (${response.status}): ${errorMessage}`);
                    return null;
                }
            } catch (error) {
                await Logger.error(`Request failed: ${(error as Error).message} (attempt ${attempt + 1}/${retries})`);
            }
        }
    ...
    async makeTranslations(config: TranslationApiConfig): Promise<ApiResponse<any> | null> {
    ...
    async fetchTranslations(params: Record<string, any> = {}): Promise<ApiResponse<any> | null> {
    ...
    async syncSources(data: SyncSourcesParams): Promise<ApiResponse<any> | null> {
        ...
        return await this.makeRequest<any>("POST", "/v1/translations/load-sources", params, data.data);
    }

    async getTranslations({ target_lang }: GetTranslationParams): Promise<ApiResponse<any> | null> {
        const params = {
            target_lang: target_lang,
        }
        return await this.makeRequest<any>("GET", "/v1/translations/language-project", params);
    }
